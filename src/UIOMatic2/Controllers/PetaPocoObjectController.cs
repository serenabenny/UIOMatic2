...
namespace UIOMatic.Controllers
{
    ...
    public class PetaPocoObjectController : UmbracoAuthorizedJsonController, IUIOMaticObjectController
    {
        ...
        public IEnumerable<UIOMaticPropertyInfo> GetAllProperties(string typeName, bool isEdit, bool isList, bool includeIgnored = false)
        {
            ...
            foreach (var prop in currentType.GetProperties())
            {
                ...
                if (includeIgnored || isList || (!isList && attris.All(x => x.GetType() != typeof(UIOMaticIgnoreFieldAttribute))))
                {
